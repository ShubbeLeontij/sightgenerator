...
Если Вы хотите узнать какие типы прицелов использует танк, можете открыть data.json - файл, по которому строятся прицелы. Каждый ключ верхнего уровня - unit_id танка.
Внутри него "zoom" - зум прицела наводчика без приближения, а "convergence" - сведение в метрах (дистанция нулевого параллакса); каждый остальной ключ - это тип прицела для генерации, содержащий начальную скорость снаряда "velocity" в м/с и, если параллакс не нулевой (обычно кроме AB/RB), пару "coords" - высоту/боковое смещение прицела относительно ствола в метрах. Пустой объект `{}` означает, что для этого танка прицел не генерируется.
...
В главном меню присутствуют столбцы выбора типов прицела (категории из data.json) на случай если вы не хотите засорять ненужными.
